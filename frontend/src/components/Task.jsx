  ...
  const handleSubmit = (e) => {
    ...
    fetch(`${import.meta.env.VITE_BACKEND_URL}/api/profile`, {
      ...
      .then((userData) => {
        ...
        if (editIndex === -1) {
          return fetch(`${import.meta.env.VITE_BACKEND_URL}/api/tasks`, {
            method: "POST",
            headers: {
          ...
          return fetch(`${import.meta.env.VITE_BACKEND_URL}/api/tasks/${taskToUpdate._id}`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
    ...
    if (window.confirm("Are you sure you want to delete this task?")) {
      ...
      fetch(`${import.meta.env.VITE_BACKEND_URL}/api/profile`, {
        ...
        .then((userData) => {
          ...
          return fetch(`${import.meta.env.VITE_BACKEND_URL}/api/tasks/${taskToDelete._id}`, {
            method: "DELETE",
    ...
    if (task && task.status !== newStatus) {
      if (currentUser && 
        ...
        if (task.status === 'Done' && newStatus === 'In Progress') {
          alert('Members cannot move tasks from Done back to In Progress');
          return;
        }
        
        if (task.status === 'In Progress' && newStatus === 'To Do') {
          alert('Members cannot move tasks from In Progress back to To Do');
          return;
        }
      }
      ...
      fetch(`${import.meta.env.VITE_BACKEND_URL}/api/profile`, {
        ...
        .then((userData) => {
          ...
          return fetch(`${import.meta.env.VITE_BACKEND_URL}/api/tasks/${taskId}`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
          ...
          if (!res.ok) {
            if (res.status === 403) {
              return res.json().then(errorData => {
                throw new Error(errorData.message || 'Operation not allowed');
              });
            }
        ...
        .then((data) => {
          setTasks((prev) => {
            const updatedTasks = prev.map((t) => (t._id === taskId ? data : t));
