...
app.put('/api/tasks/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (req.user.role === 'member') {
      ...
      if (oldTask.status === 'Done' && status === 'In Progress') {
        return res.status(403).json({ message: 'Members cannot move tasks from Done back to In Progress' });
      }
      
      if (oldTask.status === 'In Progress' && status === 'To Do') {
        return res.status(403).json({ message: 'Members cannot move tasks from In Progress back to To Do' });
      }
    }
...
app.get('/api/profile', authenticateToken, async (req, res) => {
  try {
    const user = await userModel.findById(req.user.userId).select('name email role team');
    if (!user) {
...
app.get('/api/users', async (req, res) => {
  try {
    const users = await userModel.find().select('name email role team');
    res.json(users);
  } catch (error) {
...
app.put('/api/users/:id/team', authenticateToken, requireAdmin, async (req, res) => {
  const { id } = req.params;
  const { team } = req.body;
  if (!['A', 'B', null].includes(team)) {
